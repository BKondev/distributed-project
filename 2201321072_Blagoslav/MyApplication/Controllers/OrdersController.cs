...
using ManagementApp.ViewModels.Orders;
...
namespace ManagementApp.Controllers
{
    public class OrdersController : Controller
    {
        ...
        public async Task<IActionResult> Add()
        {
            ...
            vm.Employees = EmployeesResponse.Select(EmployeeResponse => GenerateEmployee(EmployeeResponse)).ToList();
        ...
        public async Task<IActionResult> Add(AddVM addVM)
        {
            try
            {
                var response = await OrderService.Instance.PostAsync<OkResult>(new CreateOrderRequest(addVM.Details, addVM.Quantity, addVM.Tip, addVM.Total, addVM.IsExpress, DateTime.Now, addVM.Customer_ID, addVM.Employee_ID));
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            if (EmployeesResponse == null)
                return BadRequest("Couldn't load Employees which are needed for editing orders. Responce message from the server is null");
            ...
            {
                var selected = Employee.Id == vm.Order.Employee_ID ? "selected=\"selected\"" : "";
                optionsHtml.Append($"<option {selected} value=\"{Employee.Id}\">{Employee.FirstName} {Employee.LastName}</option>");
            }
            ViewBag.EmployeeOptions = optionsHtml.ToString();
        ...
        public async Task<IActionResult> Edit(EditVM vm)
        {
            try
            {
                var response = await OrderService.Instance.PutAsync<OkResult>(vm.Order.Id, new UpdateOrderRequest(vm.Order.Id, vm.Order.Details, vm.Order.Quantity, vm.Order.Tip, vm.Order.Total, vm.Order.IsExpress, vm.Order.PlacedOn, vm.Order.Customer_ID, vm.Order.Employee_ID));
        ...
        private Employee GenerateEmployee(EmployeeResponse EmployeeResponse)
        {
            return new Employee()
            {
